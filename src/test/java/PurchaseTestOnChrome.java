...
public class PurchaseTestOnChrome {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ExtentTest testChrome = extent.createTest("PurchaseTestOnChrome","Execute a purchase action on Mercedez-benz portal");
		...
		testChrome.log(Status.INFO,"Starting Test Case");
		driver.manage().window().maximize();
		driver.get(MERCEDES_URL);
		testChrome.pass("Browser was successfully reached");
		waitUntilPageLoad();
		...
		waitUntilPageLoad();
		testChrome.pass("Product's list successfully displayed");
		driver.findElement(By.xpath(BEACH_BAG)).click();
		waitUntilPageLoad();
		testChrome.pass("Beach bag was successfully selected");
		driver.findElement(By.xpath(ADD_TO_BASKET)).click();
		Assert.assertEquals(driver.getTitle(),"Beach bag");
		testChrome.pass("Beach bag was added to basket, and title validation was successfully executed");
		waitUntilPageLoad();
		driver.findElement(By.xpath(GO_TO_SHOPPING_BASKET)).click();
		waitUntilPageLoad();
		testChrome.pass("Shopping basket is displayed");
		...
		testChrome.pass("Purchase flow was executed with success");
		driver.close();
		testChrome.pass("Close the browser");
		testChrome.info("Test completed");
